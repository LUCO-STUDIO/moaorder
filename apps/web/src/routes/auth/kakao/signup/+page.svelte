<script lang="ts">
	...
	type Step = 'name' | 'birthdate' | 'region' | 'consents';
	const STEP_ORDER: Step[] = ['name', 'birthdate', 'region', 'consents'];
	...
	>
		<div class="w-full max-w-sm rounded-t-2xl bg-background p-5 sm:rounded-2xl">
			...
			<p class="mb-5 text-sm leading-relaxed text-muted-foreground">
				지금 나가시면 입력하신 정보가 사라지고, 카카오 인증부터 다시 진행해야 해요.
			</p>
